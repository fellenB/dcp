...
def _init_(args):
    ...
    if platform.system() == "Windows":
        ...
        os.system(
            "copy data.py checkpoints" + "/" + args.exp_name + "/" + "data.py.backup"
        )
    else:
        os.system(
            "cp main.py checkpoints" + "/" + args.exp_name + "/" + "main.py.backup"
        )
        os.system(
            "cp model.py checkpoints" + "/" + args.exp_name + "/" + "model.py.backup"
        )
        os.system(
...
def main():
    ...
    parser.add_argument(
        "--num_points", type=int, default=1024, metavar="N", help="Num of points to use"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    if args.eval:
        ...
        pointcloud = o3d.io.read_point_cloud("data/bottle_milktea.pcd")
        print(pointcloud)
        pointcloud_model = o3d.io.read_point_cloud("data/bottle_milktea_model.pcd")
        print(pointcloud_model)
        pointcloud_sence = o3d.io.read_point_cloud("data/bottle_sence.pcd")
        print(pointcloud_sence)
        ...
        # pointcloud_down =pointcloud.voxel_down_sample(voxel_size=0.001)
        # print(pointcloud_down)
        # o3d.visualization.draw_geometries([pointcloud_model,pointcloud])
        # o3d.io.write_point_cloud("copy_of_fragment.pcd", pcd)
        pointcloud_model = pointcloud_model.voxel_down_sample(voxel_size=0.003)
        print(pointcloud_model)
        # pointcloud_model = copy.deepcopy(pointcloud_model).translate((0.3, 0.5, 0.4))
        points_src = np.array(pointcloud_model.points)

        points_tgt = np.array(pointcloud.points)

        rot = [
            [0.4034, 0.7468, 0.5287],
            [-0.5599, 0.6585, -0.5029],
            [-0.7237, -0.0932, 0.6838],
        ]

        # points_tgt = np.dot(points_src, rot)

        points_src = points_src.transpose(1, 0)
        src = torch.tensor([points_src])
        src = src.float()
        # print(src)
        points_tgt = points_tgt.transpose(1, 0)
        ...
        # print(points_src)
        points_trans = np.dot(
            points_src, rotation_ab_pred[0].detach().cpu().numpy().transpose(1, 0)
        )  # 旋转后 转置 在进入算法，
        print(points_trans)
        # points_trans = points_trans + translation_ab_pred[0].detach().cpu().numpy()
        # print(points_trans)

        pcd_o3d = o3d.geometry.PointCloud()
        pcd_o3d.points = o3d.utility.Vector3dVector(points_trans)
        pc_model_trans = o3d.geometry.PointCloud()
        pc_model_trans.points = o3d.utility.Vector3dVector(points_tgt.transpose(1, 0))
        # pcd.normals = o3d.utility.Vector3dVector(nxnynz)
        # pcd.colors = o3d.utility.Vector3dVector(rgb)
        o3d.visualization.draw_geometries([pointcloud, pointcloud_model, pcd_o3d],"3d object", 640, 480, 500, 200)
